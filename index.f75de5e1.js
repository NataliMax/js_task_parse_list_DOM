...
function onNumber(el) {
    const number = el.dataset.salary.split("").filter((a)=>isFinite(a) === true).join("");
    return +number;
}
const newList = [
    ...list
...
function getEmployees(employeers) {
    ...
    for(let i = 0; i < employeers.length; i++){
        const p = new Object(employeers[i].textContent, employeers[i].dataset.position, employeers[i].dataset.salary, employeers[i].dataset.age);
        listObject.push(p);
    }
